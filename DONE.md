...
## Agent SDK 0.3.220 — Claude Opus 5 support (2026-07-25, v2026.7.25)

- [x] **CDB-028: Bridge couldn't run Claude Opus 5 — pinned Agent SDK shipped a pre-Opus-5 Claude Code** — `@anthropic-ai/claude-agent-sdk@0.3.177` bundles Claude Code **2.1.177**, whose model table has no `claude-opus-5` entry, so a `model` / `create-session.model` of `claude-opus-5` from the phone could never resolve. Bumped to **0.3.220** (Claude Code **2.1.220**, build 2026-07-24), which knows `claude-opus-5` and `claude-opus-5[1m]` (verified by grepping the model IDs out of the downloaded native binary). No code changes were needed: the four APIs we use (`setModel`, `applyFlagSettings`, `setPermissionMode`, `interrupt`) are unchanged, and `model` is a pass-through full-ID string end-to-end. `npm install` still requires `--legacy-peer-deps` (`@anthropic-ai/sdk` remains a peer dep, types-only at our usage and marked external in esbuild). Pairs with phone **CD-050**, which adds Opus 5 to the picker. Bridge-only; `tsc` clean; 142/142 tests pass; esbuild bundle clean. (commit `f5d7a34`, 2026-07-25)

## Universal stuck-session guard for AskUserQuestion (2026-06-23, v2026.6.231)

- [x] **CDB-027: Session wedges on a multi-question AskUserQuestion answered with typed answers** — A multi-**question** group (`QuestionGroupEntry`) answered with typed/manual answers hung forever: the phone shows "All responses sent" while the header stays "Waiting for your answer", and the SDK transcript has the `AskUserQuestion` tool_use with **no tool_result**. Root cause spans both sides — the phone collapses the N per-question cards (one `ask_question` entry each, shared `tool_use_id`) into a **single per-session `pendingQuestions` flag**, and its typed-answer handler routed via `sendMessage`, which only forwards to the question-input path while that flag is set *and clears it*; so the 1st typed answer routed correctly and the 2nd+ leaked to regular `sendInput`, queuing behind the turn blocked on `canUseTool` (which never resolves). The phone-side companion fix is CD-046. **The permanent fix is bridge-side and authoritative:** while a turn is blocked on a pending question the SDK can do nothing until it's answered, so any input arriving then can *only* be the answer — `SdkSessionManager.sendInput` now redirects to `sendQuestionInput` whenever `pendingQuestions` is non-empty, closing the whole answer-leak class regardless of how the phone routed the text (typed answers, image-as-answer, main-composer-instead-of-card, future routing bugs). Non-recursive (`sendQuestionInput`→`sendInput` fallback only fires when no question is pending). 2 regression tests added in `sdkSession.test.ts` (a full 3-question group answered entirely via the plain input path still resolves in order). Bridge-only; `tsc` clean; 142/142 pass. (commit `17125c7`, 2026-06-23)
...
- [x] **CDB-025: Session wedges on its 2nd+ permission card (hardcoded `seq: 0` collision)** — Found while E2E-testing the 2026.6.211 deadlock fix: a session's first permission card rendered fine, but a second (e.g. a sub-agent Write, then the parent's verify-`Read` of a path outside cwd) showed the "Waiting for approval" pill with **no tappable card** → wedge. Root cause: `core.ts` published every out-of-band entry with a constant `seq: 0` (permission card, auth error, device screenshot), and the phone dedups output by seq (`sessionStore.ts:631 if (seen.has(seq)) return`), so the 2nd entry sharing seq 0 was silently dropped. The `waiting_permission` state rides kind-30515 (no seq dedup) → pill-without-card mismatch. Screenshot site was a latent variant (multi-screenshot device-test sessions dropped all but the first). Fix: new `SdkSessionManager.nextSeq(sessionId)` (→ `++session.seqCounter`); all three `core.ts` `seq: 0` sites now use it. Bridge-only; no phone-app rebuild. 4 regression tests added in `sdkSession.test.ts`; typecheck clean, 138/138 pass. Bridge `2026.6.212`. (commit `e2efc6a`, 2026-06-21)

## Detect manual git commits (2026-06-20, v2026.6.201)

- [x] **CDB-023: `committed` badge missed commits not made by the agent** — The session "committed" flag only flipped when the **agent** ran `git commit` (regex match on the Bash tool_use stream); a commit made manually in a terminal never lit it. `SdkSessionManager` now captures the git HEAD hash at session start (`baseHead` via `git rev-parse HEAD`) and a 10s interval (`pollGitCommits`/`detectCommit`) flips `committed` once HEAD advances past `baseHead` — catching commits made any way. The agent-Bash fast path is kept (verifies against HEAD immediately, no 10s wait). Poll timer is `unref`'d and cleared on `dispose`. typecheck clean; 106 tests green. (commit `573ddcf`, 2026-06-20)

## AskUserQuestion Answer Round-Trip Fix (2026-06-20, v2026.6.20)

- [x] **CDB-022: AskUserQuestion answers broke after the 0.3.177 SDK upgrade** — Tapping an answer in CodeDeck returned `undefined is not an object (evaluating 'H.map')` to the model instead of the choice. Render was fine; the **answer round-trip** was wrong. SDK 0.3.177 tightened the `AskUserQuestion` contract: `canUseTool` must return `updatedInput` = original input (incl. `questions`) **plus** `answers` keyed by the full **question text** (`AskUserQuestionInput.answers?` / `AskUserQuestionOutput.answers` = "question text → answer string"; `annotations` "Keyed by question text"). The bridge returned the legacy `{ answers: { [header]: label } }` — dropping `questions` and keying by the short header — so the SDK's Bun/JSC result builder mapped over undefined. Fix (`sdkSession.ts`, all bridge-side, **zero UI change**): store the full original `toolInput` in `pendingQuestions`; `resolveQuestionAnswer` keys answers by question text and resolves `{ ...input, answers }`; `resolveQuestionKeypress`/`sendQuestionInput` pass the question text (via `question_index` → stored question, fallback `entry.content`). Free-text "Type your own" rides the same path. Multi-select left as a follow-up (needs UI). Added a regression test locking the new shape (questions echoed + answers keyed by text). 106 tests green; typecheck + build clean.
